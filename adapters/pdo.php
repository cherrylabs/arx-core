...
abstract class aDb {
    ...
    protected static function _parse($sQuery, $sContext = null) {
        ...
        if (!is_null($sContext)) {
            if (!is_array($sContext)) {
                $sContext = json_decode($sContext, true);
            }

            foreach ($sContext as $key => $val) {
                switch ($key) {
                    case 'type':
                        if (preg_match('/array|json|xml/i', $val)) {
                            $type = true;
                        }
                        break;
                }
        ...
        if (!empty($sContext['key'])) {
            ...
            foreach ($result as $key => $val) {
                if ($key === 'count') {
                    ...
                    $arr[$result[$key][$sContext['key']]] = $val;
                } else {
                    $arr[$result[$key]->$sContext['key']] = $val;
                }
            }
    ...
    public static function getInstance($sBase = DB_BASE, $sHost = DB_HOST, $sUser = DB_USER, $sPass = DB_PASS) {
        if (strpos($sHost, 'sqlite')) {
            $dns = 'sqlite:' . $sHost;
        } else {
            $dns = 'mysql:host=' . $sHost . ';dbname=' . $sBase, $sUser, $sPass;
        }
