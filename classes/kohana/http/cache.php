...
class kohana_http_cache
{
     ...
     *
     *      // Generate cache key
     *      $cache_key = HTTP_Cache::basiKohana_cache_key_generator($request);
     *
     * @param   Request   request
     * @return string
     */
    public static function basiKohana_cache_key_generator(Request $request)
    {
        $uri     = $request->uri();
        $query   = $request->query();
    ...
    public function __construct(array $options = array())
    {
        ...
        if ($this->_cache_key_callback === NULL) {
            $this->cache_key_callback('HTTP_Cache::basiKohana_cache_key_generator');
        }
    }
     ...
     *
     * The default generator is [HTTP_Cache::basiKohana_cache_key_generator()], which
    ...
    public function create_cache_key(Request $request, $callback = FALSE)
    {
        if (is_callable($callback))
            ...
            return HTTP_Cache::basiKohana_cache_key_generator($request);
    }
