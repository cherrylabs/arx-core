 ...
 *
 * @author	Stéphan Zych (http://monkeymonk.be/)
...
class jsondb
{
    ...
    {
        if(property_exists($this->data, $field))	return $this->data->{$field};
    ...
    {
        if(is_null($this->file) && is_null($file))	return false;
...
*/
    ...
    {
        if(is_array($array))	return (object) array_map(array('JSONFile', 'arrayToObject'), $array);
        else	return $array;
    } // arrayToObject
    ...
    public static function in_object($find, $object)
    {
        if (is_object($object)) {
            foreach ($object as $k => $v) {
                if(is_object($v))	return self::in_object($find, $v);
                if($find == $k)	return $v;
            }
        }
