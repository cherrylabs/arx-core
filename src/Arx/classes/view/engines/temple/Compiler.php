<?php namespace Arx\classes\views\engines\temple;

/**
 *
 * Temple shows your HTML very well and can dance with your template variables
 * It's inspired by the great work of Paul M. Jones on PHPSavant.
 *
 * @package Arx
 *
...
class Compiler {
     ...
     * the Temple object.  Any, or none, of the keys may be set.
     *
     * @return object Temple A Temple instance.
     *
     */
    ...
    /**
     *
     * Returns a copy of the Temple configuration parameters.
     *
     * @access public
     *
     ...
     *
     * By default, Temple does not use a compiler; use this to set your
    ...
    public function setPath($type, $path)
    {
        ...
        switch (strtolower($type)) {
            case 'template':
                ...
                break;
            case 'resource':
                // the Temple distribution resources
                $this->addPath($type, dirname(__FILE__) . '/Temple/resources/');
                break;
        }
    ...
    public function addPath($type, $path)
    {
        ...
        if (is_string($path) && ! strpos($path, '://')) {
            ...
            // to be searched first. however, Temple uses a stack,
     ...
     * @access public
     *
     * @param string $code A Temple 'ERR_*' string.
     *
                          ...
                          $trace = true)
    {
        ...
        if ($this->__config['exceptions']) {
            if (! class_exists('Savant3_Exception', $autoload)) {
                include_once dirname(__FILE__) . '/Temple/Exception.php';
            }
        ...
        // make sure the Temple error class is available
        if (! class_exists('Savant3_Error', $autoload)) {
            include_once dirname(__FILE__) . '/Temple/Error.php';
        }
    ...
    public function isError($obj)
    {
        ...
        if (! is_object($obj)) {
            ...
            // make sure the Temple error class is available for
            // comparison
            if (! class_exists('Savant3_Error', $autoload)) {
                include_once dirname(__FILE__) . '/Temple/Error.php';
            }
