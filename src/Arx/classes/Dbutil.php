...
class DBUtil
{
	...
	public static function set_connection($connection)
	{
		...
		{
			throw new \arxException('A connection must be supplied as a string.');
		}
	...
	/*
	 * Executes table maintenance. Will throw arxException when the operation is not supported.
	 *
	 * @throws	arxException
	 * @param     string    $table    the table name
	...
	protected static function table_maintenance($operation, $table, $db = null)
	{
		...
		if ($type === 'error')
		{
			logger(\arx::L_ERROR, 'Table: '.$table.', Operation: '.$operation.', Message: '.$result->get('Msg_text'), 'DBUtil::table_maintenance');
		}
		else
		{
