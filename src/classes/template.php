 ...
 *
 * @version $Id: template.php,v 1.42 2006/01/01 18:31:00 pmjones Exp $
 *
 */
 ...
 * Provides an object-oriented template system for PHP5.
 *
 * template helps you separate business logic from presentation logic
 * using PHP as the template language. By default, template does not
...
class c_template extends \Slim\View {
     ...
     * the template object.  Any, or none, of the keys may be set.
     *
     * @return object template A template instance.
     *
     */
...
#C
     ...
     *
     * @return mixed The plugin object, or a template_Error with an
    ...
    /**
     *
     * Returns a copy of the template configuration parameters.
     *
     * @access public
     *
     ...
     *
     * By default, template does not use a compiler; use this to set your
    ...
    public function addPath($type, $path)
    {
        ...
        if (is_string($path) && ! strpos($path, '://')) {
            ...
            // to be searched first. however, template uses a stack,
     ...
     * $template->assign('var1', 'something');
     * $template->assign('var2', 'else');
     *
     * // assign directly
     * $template->var1 = 'something';
     * $template->var2 = 'else';
     * </code>
     *
     * @access public
     ...
     * default template set with setTemplate().
     *
     * @return mixed The template output string, or a template_Error.
     *
     */
    ...
    protected function applyFilters($buffer)
    {
        ...
        foreach ($this->__config['filters'] as $callback) {
            ...
            // if the callback is a static template_Filter method,
            // and not already loaded, try to auto-load it.
            if (is_array($callback) &&
                is_string($callback[0]) &&
                substr($callback[0], 0, 15) == 'template_Filter_' &&
                ! class_exists($callback[0], $autoload)) {

                // load the template_Filter_*.php resource
                $file = $callback[0] . '.php';
                $result = $this->findFile('resource', $file);
                if ($result) {
                    include_once $result;
     ...
     * @access public
     *
     * @param string $code A template 'ERR_*' string.
     *
    ...
    /**
     *
     * Tests if an object is of the template_Error class.
     *
     * @access public
     *
     ...
     * template_Error, or is a subclass that template_Error. False if not.
     *
     */
    ...
    public function isError($obj)
    {
        ...
        if (! is_object($obj)) {
            // not an object, so can't be a template_Error
            return false;
        } else {
            ...
            $sub = is_subclass_of($obj, 'template_Error');
            return ($is || $sub);
        }
    }
}